...
public class FilterStreamExam02 {
    ...
    public static void main(String[] args) {
        ...
        list.stream().distinct().filter((s) -> s.equals("장희정")).forEach(System.out::println);

        System.out.println("3. filter 요소 걸러내기(장으로 시작하는 ) -------------");
        list.stream().filter((name) -> name.startsWith("장")).forEach(System.out::println);

        System.out.println("4. filter 요소 걸러내기(정으로 끝나는 ) -------------");
        list.stream().filter((name) -> name.endsWith("정")).forEach(System.out::println);

        System.out.println("5. 중복제거, 장으로 시작 , 출력하기 -------------");
        list.stream().distinct().filter((name) -> name.startsWith("장"))
