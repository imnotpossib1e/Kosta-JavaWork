...
public class MapStreamExam03 {
    ...
    public static void main(String[] args) {
        ...
        // .sum은 IntStream에서만 사용 가능하다 -> mapToInt
        int re = list.stream().mapToInt(Student::getScore).sum();
        System.out.println(re);

        //4. 점수의 평균
        System.out.println("4.점수의 평균--------");
        double avgRe = list.stream().mapToInt(Student::getScore).average().getAsDouble();
        System.out.println(avgRe);

        //5. 점수의 개수(인원수)
        System.out.println("5. 점수의 개수(인원수)--------");
        long cntRe = list.stream().mapToInt(Student::getScore).count();
        System.out.println(cntRe);

        //점수가 80 이상인 학생들의 이름만 검색
        System.out.println("점수가 80 이상인 학생들의 이름만 검색--------");
        list.stream().filter(s -> s.getScore() >= 80).map(Student::getName)
